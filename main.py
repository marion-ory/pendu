...
def load_highscore():
    ...
    try:
        with open("scores.txt", "r") as f:
            scores = [int(line.split("Score:")[1].split(",")[0].strip()) for line in f if "Score:" in line]
...
running = True
current_screen = "MENU"
new_word_input, game_score, errors_count = "", 0, 0
highscore = load_highscore()
found_letters, word_to_guess = [], ""
TIMER_DECREASE = pygame.USEREVENT + 1

# Mise à jour des zones de clic
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if current_screen == "MENU":
                difficulty = None
                if RECT_EASY.collidepoint(event.pos): difficulty = "EASY"
                elif RECT_MEDIUM.collidepoint(event.pos): difficulty = "MEDIUM"
                elif RECT_HARD.collidepoint(event.pos): difficulty = "HARD"
                ...
                if difficulty:
                    if os.path.exists("mot.txt"):
                        ...
                        if filtered:
                            ...
                            errors_count, game_score, found_letters = 0, 0, []
                            start_time = time.time()
                            current_screen = "GAME"
                            pygame.time.set_timer(TIMER_DECREASE, 5000)
                elif RECT_ADD.collidepoint(event.pos): current_screen = "ADD_WORD"
                elif RECT_QUIT_MENU.collidepoint(event.pos): running = False
        ...
        if event.type == pygame.KEYDOWN:
            if current_screen == "GAME" and event.unicode.isalpha():
                ...
                if event.key == pygame.K_RETURN and new_word_input:
                    with open("mot.txt", "a", encoding="utf-8") as f: f.write(new_word_input.upper() + "\n")
                    new_word_input, current_screen = "", "MENU"
                elif event.key == pygame.K_BACKSPACE: new_word_input = new_word_input[:-1]
                elif event.unicode.isalpha(): new_word_input += event.unicode.upper()

    if current_screen == "MENU": display_menu(screen, highscore)
    elif current_screen == "GAME": display_game(screen, word_to_guess, found_letters, errors_count, game_score, int(time.time() - start_time))
    elif current_screen == "ADD_WORD": display_add_word(screen, new_word_input)
    elif current_screen == "GAME_OVER": display_game_over(screen, word_to_guess)
    elif current_screen == "VICTORY": display_victory(screen, game_score, round(time.time() - start_time, 2))
